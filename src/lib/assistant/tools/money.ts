  ...
  parameters: {
    ...
    properties: {
      ...
      compareWithTeam: {
        type: "boolean",
        description:
          "Додати місце в команді й перцентилі за оборотом, чеком, поверненнями, простроченою та динамікою. Вмикай, коли питання про «чому гірше», «як я проти інших», «де провисаю».",
      },
    },
  },
  async run(ctx, args) {
  ...
  return {
    ...
    показники: keys
      ...
      .map((k) => ({
        ...
        моє: me[k],
        перцентиль: Math.round(me.ranks[k]!),
