/**
 * Хто зараз на маршруті — спільне джерело для карти керівника й помічника.
 *
 * Раніше все це лежало в роуті /api/admin/track/live. Винесено сюди, бо
 * друга реалізація того самого запиту неминуче розійшлася б із першою: на
 * екрані людина «мовчить», а в помічника «все гаразд» — і довіри не буде
 * до жодного з двох.
 *
...
export async function livePositions(
  day: string,
  now: Date = new Date()
): Promise<{ day: string; people: LivePerson[] }> {
  ...
  if (userIds.length === 0) return { day, people: [] };
  ...
  const minutesSince = (d: Date | null | undefined) =>
    d ? Math.floor((nowMs - d.getTime()) / 60_000) : null;
        ...
        ? {
            ...
            silentSinceStartMin:
              shift.status === "OPEN" && !point
                ? Math.floor((nowMs - shift.startedAt.getTime()) / 60_000)
                : null,
          }
        : null,
