          ...
          {cart.map((item) => (
            <div key={item.productId} className={`cart-row bg-white border border-[#EFEFEF] rounded-xl p-3 sm:p-4${removing.includes(item.productId) ? " cart-row--removing" : ""}`}>
              <div className="flex items-start gap-3">
                ...
                {/* aria-label обовʼязковий: у кнопці лише іконка, тож без назви
                    її не бачить ні читач екрана, ні автотест — при перевірці
                    кошика вона взагалі не знайшлась у переліку кнопок.
                    min-h/min-w 44px — палець на телефоні. */}
                <button
                  onClick={() => handleRemove(item.productId)}
                  aria-label={`Видалити «${item.name}» з кошика`}
                  title="Видалити з кошика"
                  className="text-[#9E9E9E] hover:text-[#0A0A0A] flex-shrink-0 flex min-h-11 min-w-11 items-center justify-center rounded-lg active:bg-[#F7F7F7]"
                >
