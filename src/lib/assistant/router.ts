...
export type Intent =
  ...
  /** start: «me» — від людини (геолокація пристрою чи трек), «depot» — від складу. */
...
/**
 * Керівник просить ЗВАЖИТИ, а не показати — це робота моделі.
 *
 * 16.09.2026 власник спитав «проаналізуй продажі за 2 місяці, врахуй
 * сезонність і підкажи, що замовити» — і шаблон «продажі + період» віддав
 * йому таблицю торгових за вересень за 0 токенів. Упевнена відповідь не на
 * те питання гірша за повільну правильну.
 *
 * Два сигнали. Слова поради й прогнозу («підкажи що», «спрогнозуй»,
 * «сезонність», «врахуй», «на наступний місяць»). І довжина:
 * типові питання, які код знає, — до десятка слів («скільки продав Кулик за
 * тиждень»); довше — це вже кілька умов, і шаблон виконає лише першу.
 * «Проаналізуй Пайду» і «що треба замовити по бренду APRO» лишаються коду:
 * вони короткі, а кодова відповідь на них повна (дефіцит бренду з плитками й
 * діаграмою).
 */
...
/*
 * «Який звʼязок між знижкою і маржею» 18.09.2026 забирала кодова відповідь про
 * знижки: таблиця правильна, але питали не про це — про залежність двох
 * показників, а її показує точкова діаграма, яку малює лише модель.
 */
...
function salesIntent(
  ...
  if (routeTo) {
    // «Побудуй маршрут від мене: …» / «…зі складу: …» — це старт, а не перша точка.
    const names = routeTo
      .replace(/^(від\s+мене|(від|зі|із|з)\s+(склад\S*|баз[иі]))\s*:?\s*/i, "")
...
function adminIntent(
  ...
  switch (base.kind) {
    ...
    /*
     * Маршрут керівник будує собі — від того місця, де він зараз (з
     * геолокації пристрою, див. assistant/here.ts). Склад — лише коли його
     * назвали: «від складу», «зі складу». Місця людини не знаємо — відповідь
     * сама відступить до складу й скаже чому.
     */
    case "ROUTE_TO":
      return { ...base, start: /(^|\s)(від|зі|із|з)\s+(склад\S*|баз[иі])(\s|:|,|$)/i.test(text) ? "depot" : "me" };
