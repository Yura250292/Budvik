...
export async function checkTrackSilence(): Promise<number> {
  ...
  /**
   * І — головне — стукаємо в САМ ПЛАНШЕТ, а не лише в чат офісу.
   *
   * Місяць єдиною порадою при мертвому треку було «зателефонуйте, хай
   * відкриє застосунок». Причина не в лінощах: застосунок, якого система
   * прибила, не може попередити навіть сам себе — він мертвий, і жодне
   * локальне нагадування з нього вже не вийде. Постукати ззовні в сплячий
   * Android може РІВНО ОДНА річ — push-сповіщення.
   *
   * І воно не просто показує текст. Доставка сповіщення заносить застосунок
   * у тимчасовий білий список системи, а в цьому вікні Android дозволяє
   * підняти службу переднього плану з фону — те, що інакше заборонено з
   * 12-ї версії. Тобто це єдина законна мить оживити запис без людини.
   *
   * Тому шлемо лише на LOST: там кілометри гинуть просто зараз. Затримка
   * доставки (DELAYED) лікується сама, і будити заради неї нікого не треба.
   *
   * Тротл спільний із Telegram — той самий рядок у SyncState вище. Другого
   * лічильника тут свідомо немає: два різні пороги дали б планшет, який
   * дзвенить частіше, ніж офіс про це знає.
   */
  await Promise.all(
    lost.map((t) =>
      sendPushToUser(t.userId, {
        /**
         * ТИХЕ, без тексту — і це не турбота про спокій людини, а технічна
         * вимога. Повідомлення з текстом Android віддає одразу в шторку й
         * застосунок не запускає взагалі; будить лише те, що без тексту.
         * Перевірено на живому планшеті: з текстом Expo двічі відзвітував
         * «доставлено», сповіщення на екрані було, а застосунок не прокинувся.
         *
         * Текст малює сам застосунок і лише тоді, коли тихе підняття не
         * вдалося (mobile/src/track/wake.ts → warnRecordingDown). Якщо
         * вдалося — людину не турбуємо взагалі: вона й не мала знати, що
         * трек падав і піднявся.
         */
        silent: true,
        urgent: true,
        /**
         * `screen` читає застосунок (mobile/src/track/notification-taps.ts),
         * якщо людина натисне сповіщення. Але головна робота відбувається
         * ще до дотику: сам факт доставки будить фонове завдання, яке
         * пробує підняти запис (mobile/src/track/wake.ts).
         */
        data: { screen: "/shift", reason: "track-lost" },
      })
    )
  );
