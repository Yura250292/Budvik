...
export type ToolContext = {
  ...
  question?: string;
  /**
   * Де людина зараз — для маршруту «від мене».
   *
   * Власник просив, щоб маршрут починався з його геолокації, а не зі
   * складу: він сам їде по точках. Джерело — браузер (координати приходять
   * у тілі запиту разом із маршрутним питанням) або, коли браузер не дав,
   * свіжа точка треку робочого застосунку. Немає ні того, ні іншого —
   * поля немає, і маршрут чесно виїжджає зі складу.
   */
  here?: Here;
};
...
export type Here = {
  lat: number;
  lng: number;
  /** Похибка, м; null — джерело її не назвало */
  accuracyM: number | null;
  source: "пристрій" | "трек";
};
