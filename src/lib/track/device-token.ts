...
/**
 * Кому взагалі видається токен пристрою — тобто хто заходить у робочу збірку.
 *
 * Назва історична: спершу цей контур був лише про трек, і список збігався з
 * тими, хто возить планшет. Складовщик нікуди не їде й точок не пише (роут
 * /api/track/points стоїть на FIELD_ROLES, і WAREHOUSE туди не входить), але
 * без токена він не міг би увійти в застосунок ВЗАГАЛІ — а саме там тепер
 * живе сканер накладних.
 *
 * Тобто цей список відповідає на питання «чи пускати в застосунок», а що
 * саме людині там можна — вирішує роль на кожному роуті окремо.
 */
export const TRACK_ROLES = ["DRIVER", "SALES", "ADMIN", "MANAGER", "WAREHOUSE"];
