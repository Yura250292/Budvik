...
console.log("\nВідрізок, підбитий слабкими фіксами");
{
   ...
   * йшли прямими. Такий відрізок добирається дорогою — але вести її ЧЕРЕЗ
   * самі слабкі фікси не можна.
   *
   * Фікси нижче лежать уздовж прямої між опорами: кожен відхиляється від
   * неї на десятки метрів при власній похибці в 550-700. Сказати «дорога
   * йшла сюди» вони не можуть — а OSRM зобов'язаний пройти крізь кожну
   * задану точку, і саме так у Щирці 01.09 маршрут з'їжджав із траси,
   * обходив квартал і вертався. Пробіг від цього ріс: 31.08 у Валентина
   * розриви з прямою 22 км дали «дорогою» 334 км.
   */
  const r = preparePoints([
    { lat: 49.8555, lng: 24.0325, recordedAt: t(0), accuracyM: 10 },
  ...
  check("Фікси вздовж прямої в маршрут не йдуть", gaps[0].via.length, 0);
  ok("Початок — надійна опора, а не сусідня слабка", gaps[0].from.lat === 49.8555);
}
{
  /**
   * Зворотний бік того самого правила: гак, заради якого проміжні точки й
   * вводилися (44476c2). Фікс слабкий, але стоїть за два кілометри вбік
   * від прямої — власною похибкою в 600 м це не пояснити, отже дорога
   * справді туди заходила, і маршрут має піти через нього.
   */
  const r = preparePoints([
    { lat: 49.8555, lng: 24.0325, recordedAt: t(0), accuracyM: 10 },
    { lat: 49.8277, lng: 24.0491, recordedAt: t(10), accuracyM: 600 },
    { lat: 49.8000, lng: 24.0100, recordedAt: t(20), accuracyM: 10 },
  ], null);

  const gaps = findGaps(r.points);
  check("Справжній гак лишається проміжною точкою", gaps[0].via.length, 1);
  ok("І це саме той фікс", Math.abs(gaps[0].via[0].lng - 24.0491) < 1e-9);
}
{
