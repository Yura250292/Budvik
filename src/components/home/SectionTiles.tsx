...
export default function SectionTiles({
  ...
  return (
    <div className={className}>
      ...
      {/*
        Смуга, що тягнеться, а не сітка з фіксованими колонками.

        Розділів тут стільки, скільки лишилось після банерів, і це число
        змінюється разом із каталогом. Будь-яка сітка з рівними колонками
        лишає в останньому ряду дірку — при чотирнадцяти плитках на дев'ять
        колонок це п'ять порожніх клітинок завширшки в пів екрана. Тепер
        плитки останнього ряду просто розтягуються на вільне місце: 8.5rem —
        не ширина, а мінімум, від якого вони ростуть.
      */}
      <div className="flex flex-wrap gap-2 sm:gap-3 [&>a]:flex-[1_1_5.5rem] sm:[&>a]:flex-[1_1_8.5rem]">
        {tiles.map((t) => (
          <Link
            key={t.id}
            href={t.href}
