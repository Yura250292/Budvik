...
/**
 * Швидкість, з якої це вже не помилка приймача, а не та планета.
 *
 * 03.09 планшети Ігоря й Олександра півгодини звітували Ліму: 48 і 1 точка з
 * похибкою ±3–9 м, зі швидкістю понад 150 км/год і стрибками по 1,2 км кожні
 * 22 секунди в квадраті два на два кілометри посеред перуанської столиці.
 * Приймач у такому стані не «неточний» — він рахує позицію з чужих сигналів, і
 * жодне її значення не варте нічого.
 *
 * MAX_PLAUSIBLE_KMH вище стереже лише ПРОБІГ, і свою роботу зробив: 11 830 км
 * туди й стільки ж назад у кілометри дня не пішли. Але точки лягли в трек — і
 * день на карті став лінією через Атлантику, у якій справжнього маршруту не
 * видно взагалі. Тому тут другий, грубіший поріг: не «не рахувати», а «не
 * зберігати».
 *
 * Чому не ті самі 150: справжня дірка в треку виглядає швидкою. Планшет
 * пролежав 40 секунд без фіксів, машина проїхала 800 метрів — це 72 км/год, а
 * та сама дірка з трохи збитим годинником пристрою дає й 300. 400 км/год не
 * досягає жодна дорожня ситуація, зате ловить будь-яку підміну позиції.
 */
export const IMPOSSIBLE_KMH = 400;

/**
 * Стрибок коротший за це швидкістю не перевіряємо.
 *
 * Дві точки за секунду з різницею 200 м дають 720 км/год — і це звичайне
 * тремтіння фікса, а не телепорт. Такі точки вже стереже MAX_PLAUSIBLE_KMH у
 * пробігу, а викидати їх із треку означало б рвати лінію там, де людина
 * просто їхала з поганим прийманням.
 *
 * Число перевірено на базі, а не вибрано на око. З порогом 3 км під правило
 * потрапляли двоє: у Кулика 31.08 на трасі за Дрогобичем фікс по вежі
 * (±873 м) стрибнув 3 км за 25 секунд, у Валентина 27.08 те саме сталося на
 * стоянці. Це не інша планета, це поганий приймач — і такий слід дорозі
 * потрібен, саме ним OSRM веде маршрут. 20 км же не проїде жоден бус за три
 * хвилини, зате перуанські 11 830 км ловляться з великим запасом.
 */
const IMPOSSIBLE_MIN_M = 20_000;

/**
 * Робоча рамка: за її межами координата не про роботу.
 *
 * Потрібна тому, що правило швидкості безсиле рівно в одному місці — на ПЕРШІЙ
 * точці дня. Порівнювати її ні з чим, і якби планшет прокинувся вже
 * підміненим, Ліма стала б опорою для всього дня, а справжні точки полетіли б
 * у відсів як неможливі. Рамка знімає це питання.
 *
 * Свідомо велика — уся країна з запасом, а не область: різати трек по межі
 * області означало б втратити відрядження, а таке трапляється.
 */
const HOME_BOUNDS = { latMin: 43, latMax: 53.5, lngMin: 21, lngMax: 41.5 };

/**
 * Чи не могло цієї координати бути.
 *
 * Окремою функцією, а не двома рядками в циклі, з однієї причини: те саме
 * правило потрібне разовому ремонту бази (scripts/drop-impossible-points.mts),
 * а два списки порогів розійшлися б — і скрипт чистив би не те, що відсіює
 * прийом.
 *
 * `prev` — остання ЗАПИСАНА точка будь-якої якості або null для першої точки
 * дня. Для неї лишається тільки рамка, і саме заради цього випадку рамка й є.
 */
export function isImpossibleFix(
  p: { lat: number; lng: number; recordedAt: Date },
  prev: { lat: number; lng: number; recordedAt: Date } | null
): boolean {
  if (
    p.lat < HOME_BOUNDS.latMin ||
    p.lat > HOME_BOUNDS.latMax ||
    p.lng < HOME_BOUNDS.lngMin ||
    p.lng > HOME_BOUNDS.lngMax
  ) {
    return true;
  }
  if (!prev) return false;

  const jumpM = haversineM(prev.lat, prev.lng, p.lat, p.lng);
  if (jumpM < IMPOSSIBLE_MIN_M) return false;

  const hours = (p.recordedAt.getTime() - prev.recordedAt.getTime()) / 3_600_000;
  return hours <= 0 || jumpM / 1000 / hours > IMPOSSIBLE_KMH;
}
...
export type PrepareResult = {
  ...
  rejected: {
    ...
    stale: number;
    malformed: number;
    /** Координата, якої фізично не могло бути — див. IMPOSSIBLE_KMH. */
    impossible: number;
  };
...
export function preparePoints(
  ...
  const rejected = { accuracy: 0, stale: 0, malformed: 0, impossible: 0 };
  ...
  for (const p of sorted) {
    ...
    /**
     * Координата, якої не могло бути: не та планета або телепорт.
     *
     * Міряємо від cursor — останньої ЗАПИСАНОЇ точки будь-якої якості, а не
     * від надійної опори: опора буває за годину до цього, і тоді навіть
     * стрибок через океан вкладається в «швидкість».
     *
     * Стоїть після відсіву за похибкою й до того, як точка стане опорою: інакше
     * Ліма ставала б опорою для справжніх точок, і в відсів полетіли б вони.
     */
    if (isImpossibleFix({ lat: p.lat, lng: p.lng, recordedAt: at }, cursor)) {
      rejected.impossible++;
      continue;
    }
