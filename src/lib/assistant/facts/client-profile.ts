...
export async function clientProfileFacts(counterpartyId: string, months: number) {
  ...
  const [state, aging, discipline, memory, comments, visits, brands, products, orders, summary] =
    await Promise.all([
  ...
  return {
    ...
    топ_бренди: brands.map((b) => ({
      бренд: b.brand ?? "Без бренду",
      сума: uah(b.amount),
      документів: b.docs,
    })),
    топ_товари: products.map((p) => ({
    ...
    останні_документи: orders.map((o) => ({
      номер: o.number,
