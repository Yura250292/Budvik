...
/**
 * Заголовок секції: короткий рядок перед пунктами.
 *
 * Двокрапка необовʼязкова. Раніше вимагалась — і секції знаходились лише в
 * описах, згенерованих за нашим шаблоном: із 36 перевірених карток блок
 * «Характеристики» показався на чотирьох. Описи, що приїхали з каталогів
 * постачальників, пишуть заголовок просто рядком («Склад», «Переваги»,
 * «Характеристики»), і весь список залишався суцільним текстом.
 *
 * Захист від хибних спрацювань — у тому, що виклик далі звіряє заголовок зі
 * словами «характеристик»/«комплектац» і вимагає під ним справжні пункти.
 */
function headingOf(line: string): string | null {
  const t = line.trim();
  if (!t || t.length > 40 || isBullet(t)) return null;
  const withoutColon = t.endsWith(":") ? t.slice(0, -1) : t;
  // Крапка чи кома в кінці — це вже речення, а не заголовок.
  if (/[.,;!?]$/.test(withoutColon)) return null;
