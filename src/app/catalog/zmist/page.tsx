...
/**
 * Зміст каталогу — десктопна версія, за зразком паперових каталогів
 * постачальників.
 *
...
export default async function CatalogTocPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 lg:py-10">
      <nav className="mb-5 flex items-center gap-2 text-sm text-[#9E9E9E]">
      ...
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4 border-b-2 border-[#FFD600] pb-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[#0A0A0A] lg:text-4xl">Зміст каталогу</h1>
        ...
        <Link
          href="/catalog"
          className="rounded-[10px] bg-[#FFD600] px-6 py-3 text-sm font-bold text-[#0A0A0A] transition hover:bg-[#FFC400]"
        >
          Відкрити весь каталог →
        </Link>
      </header>

      {/* Швидкий перехід по розділах — рядок закладок угорі, як на обкладинці */}
      <div className="mb-8 flex flex-wrap gap-1.5">
        {toc.sections.map((s) => (
          <a
            key={s.id}
            href={`#${s.id}`}
            className="inline-flex items-center gap-1.5 rounded-full border border-[#E0E0E0] bg-white px-3 py-1.5 text-xs font-medium text-[#555] transition hover:border-[#FFD600] hover:bg-[#FFD600]/10"
          >
            <span>{s.icon}</span>
            {s.title}
          </a>
      ...
      <div className="gap-x-10 lg:columns-2 xl:columns-3">
        {toc.sections.map((section) => (
          <section
            key={section.id}
            id={section.id}
            className="mb-7 break-inside-avoid scroll-mt-6"
          >
            <h2 className="mb-2.5 flex items-baseline gap-2 border-b border-[#EFEFEF] pb-1.5">
              <span className="text-lg">{section.icon}</span>
              <span className="flex-1 text-lg font-bold leading-tight text-[#0A0A0A]">
                {section.title}
              </span>
              <span className="text-xs font-medium text-[#9E9E9E]">{section.total}</span>
            </h2>
            ...
            <ul className="space-y-0.5">
              {section.lines.map((line) => (
                <li key={line.key}>
                  ...
                  >
                    <span className="text-sm text-[#1A1A1A] group-hover:text-[#0A0A0A] group-hover:underline">
                      {line.label}
                    </span>
