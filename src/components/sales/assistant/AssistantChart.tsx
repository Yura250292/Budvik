...
const AXIS = { fontSize: 11, fill: "var(--color-cab-t3, #8A8A87)" };
const GRID = "var(--color-cab-line, #E8E8E6)";
const INK = "#1A1A18";
...
function compact(value: number, unit: string): string {
  ...
  if (a >= 1_000_000) return `${grouped.format(Math.round(value / 100_000) / 10)} млн${tail}`;
  if (a >= 10_000) return `${grouped.format(Math.round(value / 1000))} тис${tail}`;
...
export default function AssistantChart({ spec }: { spec: ChartSpec }) {
  ...
  const legend = multi ? (
    <Legend
      ...
      iconType="circle"
      iconSize={8}
      wrapperStyle={{ fontSize: 11, color: "var(--color-cab-t2, #5C5C58)" }}
    />
  ) : null;
    ...
    chart = (
      <ScatterChart margin={{ top: 8, right: 12, bottom: 18, left: 0 }}>
        ...
        <XAxis
          ...
          label={spec.xLabel ? { value: spec.xLabel, position: "insideBottom", offset: -12, style: { fontSize: 11, fill: "#8A8A87" } } : undefined}
        />
        <YAxis
          type="number"
          dataKey="y"
