...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    ...
    suggested: {
      ...
      /**
       * Вал із 1С — підказка, а не автозапис.
       *
       * Досі його переписували руками зі звіту «Валовая прибыль вал»;
       * тепер він рахується з РегистрНакопления.ПродажиСебестоимость і
       * зійшовся зі звітом до 0,08% (Кулик — до гривні).
       *
       * Але підставляється він у поле grossUah, а не в усі чотири валютні:
       * собівартість регістр віддає одразу в гривні, тож розділити її назад
       * по валютах закупівлі неможливо. Хто веде розрахунок у валютах —
       * бачить підказку і вирішує сам; хто ні — приймає одним кліком.
       *
       * `covered` каже, яка частка обороту має відому собівартість: без неї
       * «вал 176 065» неможливо відрізнити від «вал 176 065, але порахований
       * з половини документів».
       */
      gross: Object.fromEntries(
        revenue.map((r) => [
          r.repId,
          {
            uah: Math.round(r.profit * 100) / 100,
            covered: r.amount > 0 ? Math.min(100, (r.costedAmount / r.amount) * 100) : 0,
          },
        ])
      ),
    },
