...
async function twoRounds(model: string) {
  ...
  const t0 = Date.now();
  try {
    const r1 = await streamChat({ model, apiKey: keys[flavor], messages, tools: TOOLS, toolChoice: "auto", maxTokens: 6000, effort: "off", timeoutMs: 40_000 });
    ...
    const t1 = Date.now();
    const r2 = await streamChat({ model, apiKey: keys[flavor], messages, tools: TOOLS, toolChoice: "none", maxTokens: 6000, effort: "off", timeoutMs: 40_000 });
