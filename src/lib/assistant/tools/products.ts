...
export const productSearch: ToolDef = {
  ...
  async run(ctx, args) {
    ...
    if (hits.length === 0) {
      /*
       * Кажемо моделі, що повтор УЖЕ був.
       *
       * Без цього підказка «спробуй ще раз» поверталася й на другий, і на
       * третій виклик, а кожен із них тягне за собою скорочення запиту —
       * до трьох важких запитів на чотирьох CTE. Модель слухняно ходила
       * по колу, а торговий дивився на пустий екран.
       */
      return {
        знайдено: 0,
        шукали_також: shorter === null ? "коротші варіанти запиту" : undefined,
        підказка:
          "нічого немає ні за повним запитом, ні за коротшими. Пробуй інше слово чи бренд лише якщо тут була ОПИСКА; інакше чесно скажи, що такого товару в базі немає.",
      };
    }
