...
const nextConfig: NextConfig = {
  /**
   * Корінь проєкту — явно, а не за здогадом.
   *
   * Turbopack шукає корінь по lockfile, а в домашній теці розробника лежить
   * чужий порожній `package-lock.json`. Next брав коренем ЙОГО й лаявся
   * попередженням на кожній збірці. Тепер не гадає.
   *
   * (Збірку тоді ламало інше — тека `app/` у корені, куди я поклав
   * завантажений APK: Next вважає її текою маршрутів і замість наших сторінок
   * бачив один бінарник. Типи маршрутів виходили порожні, і кожен layout падав
   * із «Type "/driver" does not satisfy the constraint never» — помилка
   * вказувала на справний файл. Тримати APK у корені не можна; він лежить в
   * output/apk/.)
   */
