 ...
 *
 * Той самий екран стоїть і в адмінці (section="admin"): там замість шапки
 * кабінету — заголовок у стилі панелі, а всі посилання лишаються в
 * /admin/catalog, інакше перший же фільтр викидав би з панелі управління.
 */
export default async function CatalogListScreen({ section, params }: { section: CatalogSection; params: SP }) {
  const { toc: tocPath, list } = catalogPaths(section);
  ...
  */
  ...
  const totalLabel = `${total.toLocaleString("uk-UA")} позицій`;
  /*
  ...
  return (
    <div className={isAdmin ? undefined : "min-h-screen bg-background"}>
      {!isAdmin && <SalesHeader title={title} subtitle={totalLabel} backTo={tocPath} />}
      ...
      */}
      <div
        className={
          isAdmin
            ? "mx-auto w-full max-w-[110rem] px-4 py-5 md:px-6"
            : "mx-auto max-w-lg px-4 pt-4 md:max-w-4xl lg:max-w-6xl xl:max-w-[110rem]"
        }
      >
        {isAdmin && (
          <div className="mb-4">
            <Link href={tocPath} className="text-[13px] text-g500 transition-colors hover:text-bk">
              ← Зміст каталогу
            </Link>
            <h1 className="mt-1 text-lg font-bold leading-tight text-bk">{title}</h1>
        ...
        */}
        <div className="mb-4">
          <SalesCatalogSearch key={filters.search ?? ""} initialQuery={filters.search ?? ""} listPath={list} />
        </div>
        ...
        <div className="flex flex-col gap-4 md:flex-row">
          ...
          <div className="min-w-0 flex-1">
            ...
            {products.length === 0 ? (
              <div className="py-16 text-center text-g400">
                ...
                */}
                {filters.search && narrowed ? (
                  <Link
                    href={`${list}?search=${encodeURIComponent(filters.search)}`}
                  ...
                  <>
                    ...
                    <Link href={list} className="mt-2 inline-block font-medium text-[#FFB800]">
              ...
              <>
                <SalesProductList products={products} cartBar={isAdmin ? "sticky" : "fixed"} />

                {totalPages > 1 && (
                  <nav className="mt-6 flex items-center justify-center gap-2 pb-4">
                    {page > 1 && (
                      <Link
                        href={`${list}${filtersToQuery(filters, page - 1, SALES_QUERY)}`}
