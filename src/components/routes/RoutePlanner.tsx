...
/**
 * Планувальник маршруту на карті — вкладка «Карта» сторінки «Маршрути».
 *
 * Був окремою сторінкою /admin/erp/route-planner і другим пунктом меню поруч
 * із маршрутами доставки, хоча працює з тими самими маршрутами: логіст ходив
 * між двома екранами й двома перемикачами «Список / Карта». Тепер це вкладка,
 * а сторінка лишилася редиректом заради закладок.
 *
 * Змінилося рівно дві речі: шапку (заголовок, перемикач, «назад») дає тепер
 * сторінка, а рольовий гейт — теж вона. Уся логіка планування — адреси
 * вручну, геокодування, склади, збережені маршрути, вибір точки кліком по
 * карті, розрахунок пального, Waze — лишилася як була.
 */
...
export default function RoutePlanner({
  ...
  /** Маршрут, під який відкрили карту (?deliveryRouteId= у вкладці «Карта») */
  ...
  useEffect(() => {
    ...
    fetch(`/api/erp/delivery-routes/${deliveryRouteId}`)
      ...
      .catch(() => {});
    // Чекати на сесію тут більше нема потреби: рольовий гейт стоїть на
    // сторінці, і компонент рендериться лише для ADMIN/MANAGER.
  }, [deliveryRouteId]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div>
      <div className="pt-1">
        <div className="flex flex-wrap items-center justify-between gap-3">
          {/* Контекст маршруту, під який відкрили карту. Без нього планувальник
              сам по собі — чернетка з адрес, і рядок порожній. */}
          <div className="min-w-0">
            {linkedDeliveryRoute ? (
              ...
              <p className="text-sm text-g500">
                Введіть адреси або відкрийте карту з картки маршруту
              </p>
            )}
          </div>
