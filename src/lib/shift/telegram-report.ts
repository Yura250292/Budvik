 ...
 *
 * Треба розрізнити два випадки, і плутати їх не можна: точки лежать у планшеті
 * (доїдуть, пробіг відновиться) чи їх не записано взагалі (кілометрів цієї
 * години не існуватиме ніколи). Перше заспокоює даремно, друге посилає шукати
 * те, що доїхало б само.
 *
 * ЧОМУ НЕ ДОСИТЬ `buffered`. Пульс шле не «скільки точок сервер не бачив», а
 * `COUNT(*)` локальної таблиці, порахований від курсора, який живе В ПАМ'ЯТІ
 * і обнуляється при кожному перезапуску контексту. Тобто одразу після
 * перезапуску число включає й усе те, що сервер давно має. 10.09 планшет
 * Олександра о 13:35 доповів 2354 — а за три години сам сказав `buf=0`, і на
 * сервер за той час не приїхало жодної точки: то були вже віддані рядки, які
 * пішли ще раз і яких сервер відсіяв як повтори.
 *
 * Тому питаємо інакше: чи має планшет фікс СВІЖІШИЙ за найновішу точку, яку
 * ми вже маємо. Немає такого — чекати нічого, хоч би що казав лічильник.
 */
async function trackGapLine(
  ...
  const [lastPoint, beat] = await Promise.all([
    ...
    prisma.deviceHeartbeat.findFirst({
      where: { userId },
      orderBy: { at: "desc" },
      select: { at: true, buffered: true, lastFixAt: true },
    }),
  ]);
  ...
  /**
   * Свіжість пульсу перевіряємо ПЕРЕД будь-яким твердженням про буфер — обома,
   * а не лише про порожній. Пульс тригодинної давності однаково не має права
   * казати ні «чекає», ні «порожньо»: він описує планшет, яким той був три
   * години тому.
   *
   * Саме на цьому 10.09 і збрехало перше видання цього рядка: Олександру
   * закрили зміну о 17:08, а число буфера взялося з пульсу о 13:35 — і
   * повідомлення пообіцяло власникові 2354 точки, яких уже не існувало.
   */
  ...
  /** Чи бачив планшет небо ПІЗНІШЕ за останнє, що маємо ми. */
  const hasNewerFix =
    beat?.lastFixAt != null && beat.lastFixAt.getTime() > lastPoint.recordedAt.getTime();

  const tail = !beatFresh
    ? `   Планшет мовчить і сам (пульсу ${beatAgeMin ?? "—"} хв): чи записалися ці кілометри — невідомо.`
    : buffered > 0 && hasNewerFix
      ? `   У планшеті чекає до ${buffered} точок: доїдуть — пробіг перерахується сам.`
      : `   Свіжих точок у планшеті немає — схоже, цих кілометрів не записано взагалі.`;
