      ...
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="flex flex-wrap items-center gap-2" style={{ marginTop: "3px" }}>
            ...
            {/*
              Попереджаємо лише про пін, поставлений ЗА НАЗВОЮ МІСТА.
              Було «точка приблизна» на все, крім поставленого рукою, — тобто
              майже на кожному рядку. Напис, який стоїть скрізь, не попереджає
              ні про що; а от координата до міста — це справді центр села
              замість воріт магазину.
            */}
            {stop.geoSource === "CITY" && (
              <span style={{ fontSize: "11px", color: "#D97706" }}>пін лише до міста</span>
            )}
            {/* Без координат дорогу не побудувати — краще сказати це прямо,
                ніж мовчки не показати кнопку «Їхати». */}
            {stop.lat == null && (
              <span style={{ fontSize: "11px", fontWeight: 600, color: "#B45309" }}>
                немає координат — навігатор не поведе
              </span>
            )}
            {/* Номер із маршрутного листа: у списку нумерація наскрізна, а
                диспетчеру називають саме документний номер. */}
            {stop.sheetSeq !== stop.sequence && (
              <span style={{ fontSize: "11px", color: "#9CA3AF" }}>у листі №{stop.sheetSeq}</span>
            )}
          </span>
        </span>
      </button>
