...
export async function applySalesDocuments(
  ...
  for (const rec of records) {
    ...
    // реальних кількостей.
    //
    // РЕАЛІЗАЦІЇ — теж, і саме заради складу.
    //
    // Раніше непроведена реалізація сюди не потрапляла: на екрані торгового
    // вона справді сміття, якого він не пояснить. Але накладну набирають
    // ПРОТЯГОМ ДНЯ — менеджер дописує позиції по одній, а склад збирає їх у
    // міру появи й закінчує ще до того, як хтось поставить «проведено». Доки
    // діяло старе правило, склад цієї роботи не бачив узагалі: документ
    // з'являвся на сайті вже зібраним і відвантаженим.
    //
    // Безпечно це тому, що вся виручка читається через SOURCE_FILTER
    // (status = 'CONFIRMED'), тобто чернетка нікуди не рахується. Єдине, що
    // довелося прикрити окремо, — «хто веде клієнта»: там останній документ
    // обирався без огляду на статус, і чернетка перекидала б клієнта на
    // чужого торгового (див. facts/staff.ts, orders-today.ts, route-habits.ts).
    //
    // Повернення лишаються за старим правилом: їх ніхто не збирає.
    //
