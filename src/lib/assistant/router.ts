...
function subjectAfter(text: string, re: RegExp): string | null {
  ...
  const rawTail = clean(text.slice(match.index + match[0].length))
    ...
    /*
     * Склад ПЕРЕД назвою: «пошукай на складі який є дріт для зварювання».
     * Хвіст із протилежного боку знімається нижче, а цей бік доти не
     * знімався взагалі — і питання не впізнавалося жодним шаблоном, тож
     * ішло в модель за дев'ять тисяч токенів замість безкоштовної
     * відповіді кабінету.
     */
    .replace(/^(на\s+складі|на\s+склад|складі|складу|в\s+наявності|у\s+наявності|по\s+складу)\s+/i, "")
    /*
     * Два питальні звороти поспіль: «подивись СКІЛЬКИ Є кругів 125».
     * Шаблон зупиняється на першому дієслові, а решта питання лишається
     * в хвості — і той відкидається як «починається з питального слова».
     */
    .replace(/^скільки\s+(ще\s+)?(є|залишилось|лишилось|залишилося|на\s+складі)\s+/i, "")
...
function salesIntent(
  ...
  const card =
    ...
    // «Знайди клієнта Химич» — слово «клієнта» сказане прямо, і гадати
    // між довідниками не треба. Без цієї гілки питання йшло в пошук
    // товару, і клієнт знаходився лише запасним ходом усередині відповіді.
    subjectAfter(text, /(знайди|покажи|пошукай|шукай|найди)\s+(мені\s+)?(клієнт[а-яіїєґ]*|контрагент[а-яіїєґ]*|магазин[а-яіїєґ]*)\s+/i) ??
