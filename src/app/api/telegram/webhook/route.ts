...
export async function POST(req: NextRequest) {
  ...
  if (/^\/start\b/.test(msg.text.trim())) {
    await sendTelegramMessageResult(
      String(chatId),
      "Цей бот надсилає сповіщення з сайту Budvik27.\n\n" +
        "Щоб отримувати замовлення, відкрийте персональне посилання або " +
        "введіть 6-значний код — і те, і те дає адміністратор у розділі " +
        "«Сповіщення про замовлення».\n\n" +
        // Водіям бот маршрути шле за telegramId, який адмін вписує в картці
        // користувача. Взяти його більше нізвідки, тому називаємо самі.
        `Ваш Telegram ID: <b>${from.id}</b> — назвіть його адміністратору, ` +
        "щоб отримувати сюди маршрути."
    );
  }
