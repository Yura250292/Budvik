...
/**
 * Як назвати обрив треку.
 *
 * Окремою функцією, бо потрібна двічі — і в гілці «пульсу теж немає», і в
 * гілці «пульс є, а координат немає». Розійдуться тексти — і той самий
 * випадок читатиметься в Telegram і на карті по-різному.
 *
 * «На ходу» — не прикраса: обрив на стоянці означає втрачені хвилини, обрив
 * у русі означає втрачені кілометри, і це різні за терміновістю новини.
 */
function trackBreak(minutes: number, speedKmh: number | null | undefined): string {
  return speedKmh != null && speedKmh > MOVING_KMH
    ? `Трек обірвався ${minutes} хв тому НА ХОДУ (${speedKmh} км/год) — кілометри втрачаються`
    : `Трек стоїть ${minutes} хв`;
}
