...
export async function planDay(repId: string, day: string, limit = 30): Promise<DayPlan> {
  ...
  for (const [i, stops] of clusters.entries()) {
    ...
    directions.push({
       ...
       * так само, як і його. Далі — робота з асортиментом: хто відстає від
       * ритму, кого треба відновити, кому розширити полицю.
       *
       * БОРГУ У ВАЗІ НЕМАЄ НАВМИСНО (вимога власника 05.09.2026). День,
       * складений за розміром прострочки, перетворюється на обʼїзд
       * боржників — а торговий їде продавати. Гроші забираються по дорозі,
       * і саме так вони й показані: окремим блоком, а не приводом їхати.
       */
      score: habitual * 8 + all.filter((c) => isSalesWork(c.дія)).length * 3 + all.length * 2,
    });
  }
...
/** Дія, заради якої варто їхати продавати, а не забирати гроші. */
function isSalesWork(action: string | null): boolean {
  return Boolean(action) && !/борг|дебітор/i.test(action!);
}
