...
export type Vendor = {
  ...
  /**
   * Як звести артикул до порівнюваного вигляду саме в цього джерела.
   *
   * Типово досить normArticle: у більшості виробників артикул надрукований
   * точно так само, як стоїть у нас із 1С. Але в Дніпро-М той самий диск у нас
   * «72538-002», а на сайті «72538002», та ще й із хвостом комплектації
   * («41613000-8»), — тож там ключ свій. Застосовується до ОБОХ боків.
   */
  key?: (article: string) => string;
  /** Повнорозмірне фото товару. */
  photo: (html: string, url: string) => string | null;
  /**
   * Позначення моделі в назві — другий ключ, коли артикул безсилий.
   *
   * Потрібне там, де в 1С артикул сурогатний («1C-…»): шукати ним нічого, а
   * модель («GL-160SE», «CD-218Q») виробник друкує і в нашій назві, і в своїй.
   * Функція одна на обидва боки — і на нашу назву, і на заголовок сторінки.
   */
  model?: (name: string) => string | null;
  /**
   * Назва товару, якщо сайт дає її точніше, ніж <h1> чи <title>.
   *
   * Типово рушій бере <h1>. У dnipro-m.ua заголовок домальовує скрипт, а в
   * <title> сидить «ᐅ … • Купити в Україні» — з таким хвостом і опис картки
   * виходить сміттєвим, і звірка назв бреше.
   */
  productTitle?: (html: string, url: string) => string | null;
...
/**
 * dnipro-m.ua — офіційний сайт марки Дніпро-М.
 *
 * Розвідка 27.08 записала цей сайт у непридатні («артикул не в статичному
 * HTML») — це виявилось помилкою огляду, а не сайту. Сторінка справді
 * малюється скриптом, але всі дані для нього лежать тут же, у пропах
 * Vue-компонента `<product …>`: `code` (артикул), `:photos` (повнорозмірне
 * фото), `:characteristics`, `:descriptions`. Тобто сторінка називає артикул
 * сама — головна умова зіставлення виконана.
 *
 * Нумерація в 1С і на сайті та сама, але записана по-різному: наш «72538-002»
 * на сайті «72538002», а на самому диску надруковано «Арт. 72538-002» — тому
 * ключ зіставлення зводить обидві форми до восьми цифр (див. key).
 *
 * Хвіст «-N» у коді сайту — це комплектація, а не інший товар: «41613000-8» —
 * той самий гайковерт DTW-201BC, тільки в наборі з батареєю й зарядним. Наша
 * картка при цьому буває саме «без АКБ і ЗП», тож із кількох сторінок під один
 * артикул рушій лишає ту, чия назва ближча до нашої.
 */

/** Значення пропа Vue-компонента <product …> з картки товару. */
function dmProp(html: string, name: string): string | null {
  const at = html.search(/<product\s/i);
  if (at < 0) return null;
  // Тег величезний — у пропах їдуть характеристики, описи й відгуки, — але не
  // безмежний: беремо з запасом і не читаємо всю сторінку, щоб не зачепити
  // каруселі «схожі товари» з їхніми власними артикулами.
  const tag = html.slice(at, at + 400_000);
  const m = tag.match(new RegExp(`\\s:?${name}=(["'])((?:(?!\\1)[\\s\\S])*)\\1`));
  return m?.[2] ?? null;
}

/**
 * Значення пропа в лапках — це JSON, але екранований під атрибут HTML.
 *
 * `:characteristics` заганяють у подвійні лапки і всі внутрішні перетворюють
 * на `&quot;`, а `:photos` — у одинарні, де лапки лишаються собою. Тому спершу
 * знімаємо сутності, і лише потім JSON.parse.
 */
function dmJson<T>(html: string, prop: string): T | null {
  const raw = dmProp(html, prop);
  if (!raw) return null;
  const text = raw
    .replace(/&quot;/g, '"')
    .replace(/&#0?39;|&apos;/g, "'")
...
const dniproM: Vendor = {
  slug: "dnipro-m",
  title: "DNIPRO-M",
  site: "https://dnipro-m.ua",
  brands: ["dnipro-m"],
  discover: {
    kind: "sitemap",
    urls: ["https://dnipro-m.ua/sitemap_uk.xml"],
    pageMatch: /^https:\/\/dnipro-m\.ua\/tovar\/[a-z0-9-]+\/$/,
  },
  article(html) {
    const code = dmProp(html, "code")?.trim();
    return code && /^\d/.test(code) ? code : null;
  },
  /**
   * Вісім цифр — і тільки для номерів рівно тієї форми, що в Дніпро-М.
   *
   * Обрізати «перші вісім цифр» будь-чого не можна: у базі є артикули інших
   * постачальників, довші за вісім знаків, і таке обрізання зліпило б докупи
   * різні товари. Тому шаблон вузький: 5+3 цифри з дефісом або без і
   * необов'язковий хвіст комплектації.
   */
  key(s) {
    const a = normArticle(s);
    const m = a.match(/^(\d{5})-?(\d{3})(?:-\d{1,2})?$/);
    return m ? `${m[1]}${m[2]}` : a;
  },
  productTitle: (html) => dmProp(html, "title")?.trim() || null,
  /**
   * Перша модель у назві — і тільки перша.
   *
   * У наборі «шліфмашина DGA-201BC Ultra + акумулятор BP-240 + зарядний
   * FC-230» друга й третя моделі ведуть на сторінки акумулятора й зарядного,
   * і на картку шліфмашини приїхало б фото батареї. З іншого боку те саме
   * правило відкидає «Цанги для фрезера ER-70S»: там перша модель належить
   * цангам, а не фрезеру.
   *
   * normArticle — бо в 1С позначення набране впереміш: «ВС-16» кирилицею
   * виглядає як «BC-16», але це різні рядки.
   */
  model(name) {
    for (const m of name.toUpperCase().matchAll(/(?<![\p{L}\p{N}])([A-ZА-ЯІЇЄҐ]{2,4})[-\s]?(\d{2,4})([A-ZА-ЯІЇЄҐ]{0,3})(?![\p{L}\p{N}])/gu)) {
      // «Хомут пластиковий білий 4,8 мм 350 мм» — тут «мм 350» виглядає як
      // модель «ММ-350», і картка білого хомута 4,8 мм зіставилась із чорним
      // 4,6 мм, бо в тій назві теж є «мм 350». Одиниці виміру — не моделі.
      if (/^(ММ|СМ|КГ|ГР|МЛ|ШТ|УП|ВТ|КВТ|ОБ|ХВ|АГ|АЧ|MM|CM|KG|ML|KW|AH|MAH|BAR|PSI|PCS)$/.test(m[1])) continue;
      return normArticle(`${m[1]}-${m[2]}${m[3]}`);
    }
    return null;
  },
  photo(html) {
    const photos = dmJson<{ origin_photo?: string }[]>(html, "photos") ?? [];
    return photos.find((p) => p?.origin_photo)?.origin_photo ?? null;
  },
  specs(html) {
    const groups = dmJson<Record<string, { name?: string; value?: string }[]>[]>(html, "characteristics") ?? [];
    const out: Specs = {};
  ...
  text(html) {
    const blocks = dmJson<{ description?: string }[]>(html, "descriptions") ?? [];
    const t = blocks.map((b) => strip(String(b?.description ?? "")).replace(/\s+/g, " ")).find((x) => x.length > 60);
    return t ? t.slice(0, 900) : null;
  },
  /**
   * Картка мусить сама назватися Дніпро-М.
   *
   * Це не причіпка: у sync саме ourProduct вирішує долю картки, чий бренд у
   * базі інший, а восьмизначна нумерація перетинається з чужими артикулами.
   * Слабка перевірка «аби не аналог» пропустила б туди що завгодно.
   *
   * «під Дніпро-М» і «аналог» — китайські замінники, їм фото оригіналу не
   * належить. Межу слова пишемо руками: \b у JS кирилиці не бачить, а «під»
   * без неї знайшлося б у «підшипнику» й «піддоні».
   */
  ourProduct: (n) =>
    /(dnipro|дніпро|днiпро)/i.test(n) &&
    !/(^|[^а-яіїєґ'ʼ])(під|пiд|под|аналог)([^а-яіїєґ'ʼ]|$)/i.test(n) &&
    !/кит\./i.test(n) &&
    // «РЕМОНТ Dnipro-M Перфоратор бочковий BH-10» — це послуга ремонту, а не
    // інструмент; фото товару на такій картці обіцяє покупцю не те.
    !/(^|\s)ремонт(\s|$)/i.test(n),
};

export const VENDORS: Vendor[] = [apro, sila, makita, polax, gradient, ultra, revolt, somafix, mastertool, dniproM];
