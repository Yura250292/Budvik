  ...
  return (
    <div className="mt-10">
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <h3 className="text-xl font-bold text-gray-900">Сумісні аксесуари та витратні матеріали</h3>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {accessories.map((acc) => (
          <Link
          ...
          >
            <div className="h-32 bg-gray-50 flex items-center justify-center">
              {acc.image ? (
                <img src={acc.image} alt={acc.name} className="h-full w-full object-contain p-2" loading="lazy" />
              ) : (
            ...
            <div className="p-2.5">
              {acc.category && !/^\d+$/.test(acc.category.name) && (
                <span className="text-[10px] text-gray-400 uppercase tracking-wide">{acc.category.name}</span>
              )}
              <h4 className="font-medium text-xs text-gray-900 group-hover:text-yellow-600 transition line-clamp-2 mt-0.5 mb-1">
                {acc.name}
              </h4>
              {acc.reason && (
                <p className="text-[10px] text-gray-400 line-clamp-2 mb-1.5">{acc.reason}</p>
              )}
              <span className="text-sm font-bold text-gray-900">{formatPrice(acc.price)}</span>
            </div>
          </Link>
        ))}
      </div>
