      ...
      */}
      ...
      <WebView
        ...
        onShouldStartLoadWithRequest={(req) => {
          ...
          /**
           * Підключення Google Календаря — цілком у системному браузері.
           *
           * Google навмисно не пускає OAuth усередину WebView і віддає екран
           * помилки disallowed_useragent. Причому відкривати системно треба
           * САМЕ початок потоку, а не сторінку згоди: кука з nonce і сесія
           * мусять бути в тому ж браузері, куди Google поверне людину, —
           * інакше підключення провалиться на перевірці.
           */
          if (/\/api\/calendar\/google\/connect(\?|$)/.test(req.url)) {
