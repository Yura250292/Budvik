...
export type InsightSeverity = "critical" | "warning" | "watch" | "info" | "positive";
export type InsightUnit = "uah" | "pct" | "count" | "days";

/**
 * Звідки взяті числа інсайту. Кожне значення відповідає конкретному блоку
 * інтерфейсу, куди можна «провалитись» і побачити сирі дані:
 * перші чотири — блоки профілю торгового, решта — вкладки порівняння команди.
 */
...
export type Insight = {
  ...
  detail: string;
  /**
   * Який період описано або які вікна порівняно, датами з фактів.
   * Опційне лише через старі збережені звіти — нові генерації його мають.
   */
  period?: string;
  /** Блок даних, з якого взяті числа. Старі звіти його не мають. */
...
/**
 * Джерела даних за видом аналізу. Enum у схемі підганяється під kind:
 * модель не повинна мати змоги позначити командний інсайт блоком профілю.
 */
const SOURCES_BY_KIND: Record<"rep" | "team", { values: InsightSource[]; description: string }> = {
  rep: {
    values: ["summary", "dynamics", "portfolio", "receivables", "returns"],
    description:
      "summary — підсумок періоду (оборот, реалізації, чек); dynamics — темп, по місяцях і тижнях; portfolio — портфель клієнтів (нові/збиваються/сплять/втрачені); receivables — дебіторка; returns — повернення товару від покупців.",
  },
  team: {
    values: ["benchmark", "strengths", "brands"],
    description:
      "benchmark — показники кожного й наскільки він попереду команди; strengths — сильні та слабкі сторони; brands — прогалини по брендах.",
  },
};
 ...
 */
function reportTool(kind: "rep" | "team"): Anthropic.Tool {
  const sources = SOURCES_BY_KIND[kind];
  return {
        ...
        items: {
          ...
          properties: {
            severity: {
              ...
              description:
                "critical — втрачаємо гроші вже зараз; warning — негативна тенденція, підтверджена цифрами; watch — поки в межах норми, але варто тримати на оці; info — варте уваги, без оцінки; positive — те, що працює добре.",
            },
            title: {
              type: "string",
            ...
            period: {
              type: "string",
              description:
                "Який період описано або які вікна порівняно. Дати брати ДОСЛІВНО з даних, напр. «останні 4 тижні (2026-07-14 — 2026-08-10) проти попередніх (2026-06-16 — 2026-07-13)» або «весь період 2026-05-01 — 2026-08-11». Не вигадувати дат.",
            },
            source: {
              type: "string",
              enum: sources.values,
              description: `Блок даних, з якого взяті числа інсайту: ${sources.description}`,
            },
            evidence: {
              type: "array",
              description:
          ...
          required: ["severity", "title", "detail", "period", "source", "evidence"],
        },
      },
    },
    required: ["insights"],
...
6. У полі period кожного інсайту вкажи, який період описано або які вікна
   порівняно — дати копіюй із даних дослівно, не вигадуй і не зсувай їх.
   Якщо порівнюються два вікна, назви обидва.
7. У полі source вкажи блок даних, з якого взяті числа, — за ним керівник
   провалиться в сирі дані й перевірить висновок.

ПОВЕРНЕННЯ ТОВАРУ. Оборот скрізь НЕТТО: повернення від покупців уже
віднято. Тому «оборот» — це не «продав», а «продав мінус повернули».
Причини повернення в даних НЕМАЄ (1С її не передає) — тож констатуй факт
і концентрацію, але не пояснюй, чому повернули. Дивись не на суму саму
по собі, а на два питання: яка частка від обороту і чи сидять повернення
в одного клієнта. Разові повернення — норма торгівлі, не проблема.
...
портфель клієнтів за станами, дебіторку і повернення товару.
...
поле «позаду_відсотків_команди», медіани й матрицю «бренд × торговий».

«Позаду_відсотків_команди» — яка частка колег слабша за цією метрикою: 100
означає найкращий у команді, 0 — найгірший. Для простроченої дебіторки,
втрачених клієнтів і частки повернень шкала перевернута: високе значення
означає, що показник добрий (тобто борг малий, повернень мало).
Слова «перцентиль» не вживай — пиши «позаду 72 % команди» або «краще за
нього лише двоє».
...
Важливо: не роби висновків з абсолютних чисел без огляду на місце в команді.
