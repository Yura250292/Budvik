...
import { catalogPaths, type CatalogSection } from "./paths";
 ...
 * З тієї ж причини в адмінці той самий екран стоїть на /admin/catalog:
 * section="admin" прибирає шапку кабінету й QR торгового, а посилання веде
 * в адмінський список, щоб людина не випадала з панелі управління.
 *
 * Усі посилання звідси — prefetch={false}. На сторінці їх близько 250, і
 * всі ведуть у список, який через searchParams рендериться наживо. З
 * типовим prefetch Next смикає кожне посилання, щойно воно потрапляє в
 * екран: одне відкриття змісту й прокрутка давали під дві сотні запитів у
 * динамічний роут за кілька секунд. У цьому залпі тонув запит
 * /api/auth/session, а next-auth будь-яку невдалу відповідь читає як
 ...
 */
export default async function CatalogTocScreen({ section }: { section: CatalogSection }) {
  const [toc, tree] = await Promise.all([getCatalogToc(), getBrandTree()]);
  const { list } = catalogPaths(section);
  const isAdmin = section === "admin";
  const totalLabel = `${toc.total.toLocaleString("uk-UA")} позицій`;

  return (
    <>
      {!isAdmin && <SalesHeader title="Каталог" subtitle={totalLabel} />}
      ...
      <div
        className={
          isAdmin
            ...
            : "mx-auto max-w-lg px-4 py-4 md:max-w-5xl lg:max-w-6xl"
        }
      >
        {isAdmin && (
          <div className="mb-4">
            <h1 className="text-lg font-bold leading-tight text-bk">Каталог товарів</h1>
            <p className="mt-0.5 text-[13px] text-g500">{totalLabel}</p>
          </div>
        )}

        <div className="mb-4">
          <SalesCatalogSearch listPath={list} />
        </div>
        ...
        {/*
          ...
          QR лише в кабінеті: це персональний код торгового (клієнт, що його
          відсканував, закріплюється за ним), і менеджеру роут коду не віддає.
        */}
        <div className={`mb-4 grid gap-2 ${isAdmin ? "grid-cols-1 sm:max-w-sm" : "grid-cols-2"}`}>
          <Link
            href={list}
        ...
        */}
        <div className="gap-x-8 md:columns-2 lg:columns-3">
          {toc.sections.map((s) => (
            <section key={s.id} id={s.id} className="mb-6 break-inside-avoid scroll-mt-4">
              ...
              <Link
                href={`${list}?section=${encodeURIComponent(s.id)}`}
                prefetch={false}
              ...
              <ul>
                {s.lines.map((line) => (
                  <li key={line.key}>
                    <Link
                      href={`${list}?type=${encodeURIComponent(line.key)}`}
                      prefetch={false}
