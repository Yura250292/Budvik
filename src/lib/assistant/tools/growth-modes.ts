...
export async function prospectsReport(args: Record<string, unknown>) {
  ...
  const point = (p: (typeof onRoute)[number]) => ({
    ...
    км: p.nearestKm,
    пін: p.precision === "ADDRESS" ? "адреса" : "лише місто",
  });
  ...
  return {
    ...
    примітка: `Точка «по дорозі» в польового торгового, чиї клієнти стоять поруч: до ${PROSPECT_RADIUS_ADDRESS_KM} км для піна за адресою, до ${PROSPECT_RADIUS_CITY_KM} км для піна лише по місту (центр населеного пункту). День — коли сусідні клієнти цього торгового зазвичай беруть товар за пів року. Категорія A–D і спеціалізація — з самої бази. Точки обласного центру (Львів) з піном лише по місту нікому не приписано: пін у центрі міста, відстань нічого не каже — їм спершу потрібна адреса на карті. Нікого не закріплює: закріпити точку за торговим — на карті клієнтів. «Поза маршрутами» — там, де польові торгові клієнтів не мають: або новий напрямок, або не наша територія.`,
  };
}
