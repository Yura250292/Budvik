...
export interface CounterpartyRecord {
  ...
  /**
   * Прапорці «Поставщик» і «Покупатель» з довідника 1С, як вони там стоять.
   *
   * Тип вище виводить сервер саме з них, а не агент: доки в запиті агента
   * колонки постачальника не було, на її місці читалась позначка видалення,
   * і шість помічених контрагентів встигли стати постачальниками. Тепер
   * ПРИСУТНІСТЬ цих полів і є ознакою, що агент новий і його типу можна
   * вірити; старий агент їх не шле, і тип на сайті лишається недоторканим.
   */
  isSupplier?: boolean;
  isCustomer?: boolean;
  deleted?: boolean;
...
export interface DocumentRecord {
  ...
  posted?: boolean;
  /**
   * Позначка видалення в 1С. Приходить для замовлень і надходжень: замовлення
   * живуть на сайті непроведеними, а надходження перечитуються вузьким вікном,
   * тож викинута в 1С накладна інакше висіла б тут вічно. Поля немає (старий
   * агент) — вважаємо, що документ живий: так само, як з posted.
   */
  deleted?: boolean;
  items?: DocumentItemRecord[];
  /**
   * Склад із шапки документа (надходження) → StockLocation.externalId.
   * У замовленнях і реалізаціях цієї конфігурації складу в шапці немає.
   */
  warehouseExternalId?: string;
  /**
   * Валюта документа кодом 1С (840 USD, 978 EUR). Порожня або 980 —
   * документ гривневий, і полів валюти агент не шле взагалі.
   */
  currencyCode?: string;
  /**
   * Курс до гривні на дату документа, уже поділений на кратність (агент
   * робить те саме, що й у каналі цін). Ціни рядків і сума приходять У
   * ВАЛЮТІ ДОКУМЕНТА — перераховує їх сервер, бо це вже семантика сайту.
   * Курсу немає → сервер лишає числа як є і пише розбіжність.
   */
  currencyRate?: number;
  /**
...
export interface PaymentRecord {
  ...
  number?: string;
  /**
   * Спосіб оплати. Агент його НЕ вирішує — правило живе на сервері
   * (`classifyPaymentMethod` в apply-payments.ts), бо ПКО в 1С оформлюють
   * і на готівку, і на банківський переказ. Поле лишається в контракті
   * для ручних оплат і сумісності зі старим агентом.
   */
  method?: string;
  /**
   * Назва договору з 1С («Поставка факт», «Безготівка», «Інтернет
   * покупець»). Це головна ознака того, як прийшли гроші: банківські
   * перекази клієнтів заводять тим самим касовим ордером.
   */
  contractName?: string;
  /** Назва каси з 1С («каса грн», «БЕЗГОТІВКА ГРН», «ГРН Інтернет»). */
  cashDesk?: string;
  /**
