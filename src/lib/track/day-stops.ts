 ...
 *
 *   RouteSheet (1С) — те, що водій реально везе. Головне за задумом, але
 *   рядків там може не бути ніколи: у Документ.МаршрутнийЛист УТ 2.3
 *   табличної частини НЕМАЄ взагалі (перевірено пробою, коміт fe9debc) —
 *   документ несе лише номер, дату й водія, а список клієнтів існує тільки
 *   в друкованій формі. Код лишається, бо звідти точки прийдуть, якщо їх
 *   колись почне наповнювати обмін.
 *
 *   DeliveryRoute (сайт) — планувальник Budvik. Працює завжди, але ним
 *   користуються не щодня.
 *
 * Третій шлях у роботі: реалізації водія за датою листа. Спільних полів у
 * документа й листа лише два — водій і дата, — і чи відтворюється з них
 * склад маршруту, зараз перевіряє окрема проба в 1С. Коли підтвердиться,
 * сюди стане ще одна гілка перед DeliveryRoute.
 *
