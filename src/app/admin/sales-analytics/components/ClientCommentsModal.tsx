...
export function ClientCommentsModal({
  ...
  const openCamera = async () => {
    ...
    /**
     * Дозвіл питаємо ЩОРАЗУ, і питає його сам getUserMedia.
     *
     * Окремого «запитати дозвіл» у браузера немає: системне вікно показує
     * саме виклик getUserMedia. Тому він і стоїть першим, без жодних
     * попередніх перевірок.
     *
     * Спершу я був поставив перед ним navigator.permissions.query() — щоб
     * на «denied» одразу пояснити, де перемкнути. Проба показала, що це
     * пастка: середовище вміє віддавати «denied» там, де насправді
     * зʼявилося б вікно запиту, і тоді перевірка НЕ ДАЛА Б його показати.
     * Тобто вона могла б назавжди відрізати єдиний шлях дозволити камеру.
     * Тому стан дозволу дивимось лише ПІСЛЯ відмови — щоб уточнити текст,
     * а не щоб вирішувати за браузер.
     */
    try {
      ...
      /**
       * Відмова буває двох різних сортів, і поради до них протилежні:
       * «щойно натиснув Заборонити» лікується повторним тапом, а
       * «заблоковано для сайту» — лише в налаштуваннях, бо вікна більше
       * не буде. Розрізняє їх стан дозволу, і питаємо ми його саме тут —
       * після відмови, коли він уже нічого не може зіпсувати.
       */
      let blocked = false;
      try {
      ...
      setCameraError(
        name === "NotAllowedError" || name === "SecurityError"
          ? blocked
            ? "Камеру для цього сайту заблоковано, і браузер більше не питатиме. Меню ⋮ → «Інформація про сайт» → Дозволи → Камера → Дозволити. Якщо пункту немає: Налаштування телефона → Застосунки → Chrome → Дозволи → Камера."
            : "Доступ до камери не надано. Натисніть «Спробувати ще раз» і оберіть «Дозволити»."
