...
/**
 * Канонічна назва моделі, а не псевдонім `deepseek-chat`.
 *
 * Той псевдонім DeepSeek офіційно вимкнув 24.07.2026 і прибрав з усієї
 * документації — а відповідати він не перестав, тож помилки тут не було
 * видно. Працювати на назві, якої вже немає в переліку, означає чекати
 * 400-ї посеред робочого дня без жодного попередження.
 */
  ...
  try {
    res = await fetch(DEEPSEEK_URL, {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.3,
        /**
         * Міркування вимкнені ЯВНО, і без цього рядка зміна назви моделі
         * була б поломкою.
         *
         * У DeepSeek режим міркувань увімкнений за замовчуванням. Старий
         * псевдонім водив у режим без міркувань сам, канонічна назва —
         * ні. А в режимі міркувань `temperature` мовчки не діє (помилки не
         * буде, значення проігнорують), і токени роздуму йдуть у ту саму
         * стелю 800, тобто відповідь обрізало б на півслові.
         *
         * Тут міркування й не потрібні: цифри вже порахували запити, модель
         * їх лише переказує.
         */
