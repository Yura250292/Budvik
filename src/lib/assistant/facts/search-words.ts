...
/**
 * Апостроф у назвах товарів.
 *
 * 1С пише його трьома символами («руків'ям», «Мар`ян», «З’єднувальна»),
 * клавіатура телефона дає четвертий — «ʼ», а люди часто не пишуть зовсім:
 * «мясорубка», «зєднувальна». Підрядок таке не прощає — ні «зʼєднувальна»,
 * ні «зєднувальна» не знаходили «З’єднувальна муфта» (23.09.2026).
 *
 * Тому порівнюємо без нього з обох боків: запит — `withoutApostrophe`,
 * назву — `translate(p.name, APOSTROPHES, '')` у SQL. Мʼякий знак тут НЕ
 * чіпаємо, на відміну від контрагентів: у товарах він частина слова
 * («Кельма»), а не варіант написання.
 */
export const APOSTROPHES = "'ʼ`’´";

export function withoutApostrophe(text: string): string {
  return text.replace(/['ʼ`’´]/g, "");
}

/**
 * Слово, у якому апостроф може стояти: він уже є, або «є/ї/ю/я» стоїть
 * після іншої букви («мясорубка» — «мʼясорубка»). Лише для таких слів
 * вмикаємо порівняння без апострофа: воно обходить індекс, і платити за
 * нього на кожному слові запиту немає причини (76 мс проти 241 мс).
 */
export function mayHaveApostrophe(word: string): boolean {
  return /['ʼ`’´]/.test(word) || /[^\sєїюя][єїюя]/i.test(word);
}
